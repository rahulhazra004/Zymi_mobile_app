...
public class Cat_Image_Adapter extends RecyclerView.Adapter<Cat_Image_Adapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        holder.fsatisfy.setTag(position);
        if(selection.getFsatisfy().equals("0")){
            holder.fsatisfy.setVisibility(View.GONE);
        }else {
            holder.fsatisfy.setVisibility(View.VISIBLE);
        }

        holder.rate_count.setText(selection.getImg_rating());
