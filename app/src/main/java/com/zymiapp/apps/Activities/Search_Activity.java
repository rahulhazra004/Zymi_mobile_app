...
import com.zymiapp.apps.Adapters.Search_Adapter;
...
public class Search_Activity extends AppCompatActivity {
    ...
    private static final String SEARCH_URL = "https://www.resellingapp.com/apiv2/zymi/rest_server/searchForProducts/API-KEY/123456";
...
//        });
        ...
        if (extras!=null){
            if(intent.hasExtra("pNo")) {
                pNo = extras.getString("pNo");
            }

            if(intent.hasExtra("searchtxt")) {
                searchdata = extras.getString("searchtxt");
                autoCompleteTextView.setText(searchdata);
                lists.clear();
                //mAdapter.notifyDataSetChanged();
                hideSoftKeyboard();
                p_bar.setVisibility(View.VISIBLE);
    ...
    void search_Kurtis(){
        ...
        Log.v("Rabby","requestData:"+new JSONObject(postParam));
                        ...
                        try {
                            ...
                            if (status.equals("200")){
                                ...
                                for (int i=0;i<jsonArray.length();i++){
                                    ...
                                    String catlogue_id = properties.getString("product_catalogue_id");

                                    Fav fav = new Fav(cat_name,actual_price,price,img_url,false,img_id,catlogue_id);
                                    lists.add(fav);
                                }
                                ...
                                mAdapter = new Search_Adapter(getApplicationContext(), lists, new Search_Adapter.ClickListener() {
                                    ...
                                    public void onImgClick(View view, int position) {
                                        if (lists.get(position).getCatlogue_id().equals("0")){

                                            images = new ArrayList<>();

                                            for (int i=0;i<lists.size();i++){
                                                Selection selection = new Selection(lists.get(i).getImg_url(),lists.get(i).getSelect(),lists.get(i).getImg_id(),lists.get(i).getSelling_price(),lists.get(i).getActual_price(),lists.get(i).getCat_name(),"",false);
                                                images.add(selection);
                                            }
                                            ...
                                            intent.putExtra("cat_id",lists.get(position).getCatlogue_id());
                                            startActivity(intent);
                                        }
                                    }
                                });
